<template>
  <div class="app-container">
    ...
    <el-container style="height: 100%">
      <el-aside style="border: 1px silver solid">
        <!-- 部门 -->
        <el-tree
          :data="data"
      ...
      <el-main style="padding: 0 0 0 20px;">
        ...
        <el-row style="margin-top: 20px;">
          ...
          >
            ...
            <el-table-column label="可预约数" prop="reservedNumber" width="80" />
            <el-table-column label="剩余预约数" prop="availableNumber" width="100" />
            <el-table-column label="挂号费(元)" prop="amount" width="90" />
...
import depApi from '@/api/yygh/Department'
...
export default {
  ...
  created() {
    ...
    this.workDate = this.getCurDate()// 获取第一个日期
    this.fetchData()
  },
  methods: {
    // 查询排班详情
    ...
    fetchData() {
      depApi.getDeptByHoscode(this.hoscode)
        .then(response => {
          this.data = response.data.list // 查询当前医院的科室列表（大）
          // 默认选中第一个
          if (this.data.length > 0) {
            this.depcode = this.data[0].children[0].depcode // 第一个大科室下第一个小科室
