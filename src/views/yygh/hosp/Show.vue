    ...
    <table class="table table-striped table-condensed table-bordered" width="100%">
      <tbody>
        <tr>
          ...
          <td width="35%">
            <img :src="'data:image/jpeg;base64,'+hospital.logoData" alt="logo" width="80">
          </td>
        </tr>
        <tr>
          <th>医院编码</th>
  ...
  methods: {
    ...
    fetchHospDetail(id) {
      hospApi.getHospById(id)
        .then(resp => {
          this.hospital = resp.data.hospital.hospital
          this.bookingRule = resp.data.hospital.bookingRule
        })
    },
    // 返回医院列表
    back() {
      this.$router.push({ path: '/yygh/hosp/HospList' })
    }
  }
}
</script>
